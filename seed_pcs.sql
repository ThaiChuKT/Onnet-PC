...
-- SUBSCRIPTION PLANS (30/365 days) for each spec so detail page can display package prices
...
SELECT CONCAT(s.spec_name, ' - Monthly'), s.id, 30, ROUND(s.price_per_hour * 140, 2), NULL, TRUE
FROM pc_specs s
...
SELECT CONCAT(s.spec_name, ' - Yearly'), s.id, 365, ROUND(s.price_per_hour * 1400, 2), NULL, TRUE
FROM pc_specs s
WHERE s.spec_name IN (
  'Nebula Starter',
  'Nebula Plus',
